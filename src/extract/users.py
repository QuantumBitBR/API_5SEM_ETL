    ...
    try:
        ...
        for user in users:
            ...
            if email:
                ...
                Logger.error(f"Email not found for user {user.id} in project {project_id}.")
...
def get_useremail_by_project_by_memberships(project_id, user_id):
    ...
    if not token:
        Logger.error("No API token provided. Please set the TAIGA_API_TOKEN environment variable.")
        return None
