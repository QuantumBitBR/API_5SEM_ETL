...
def process_data_2_fact_temporais():
    ...
    for projetc in projects:
        ...
        Logger.info(f"Fetched internal project ID: {internal_project_id} for project {projetc['name']}")
        ...
        for story in stories:
            ...
            Logger.info(f"Fetched internal user ID: {internal_user_id} for user {taiga_user_id}")
            
            
            for periodos in DIM_PERIODOS:
                
                internal_key = f"{internal_user_id}-{periodos['id']}-{internal_project_id}"
                ...
                Logger.info(f"Period ID: {periodos['id']}, Start Period: {start_period}, Created Date: {created_date}, Finish Date: {finish_date}")
