        ...
        try:
            ...
            for story in stories:
                ...
                if finish_date is None:
                    Logger.warning(f"Finish date is None for story {story['id']}... skipping")
                    continue
                else:
            ...
            Logger.error(f"An error occurred while processing project {project['name']} (ID: {project['id']}): {e}")
            conn.rollback()
        finally:
            conn.commit()
            conn.close()
            Logger.info(f"Finished processing project: {project['name']} (ID: {project['id']})")
