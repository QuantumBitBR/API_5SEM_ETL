        ...
        for project in projects:
            ...
            for story in stories:
                try: 
                    ...
                    Logger.error(f"Error processing rework for story {story['id']}: {e}")
                
                Logger.info(f"Processing story {story['id']} from project {project['name']}")
...
def retrabalho(user_story, conn, cursor):
    ...
    try:
        ...
        if current_status_row is None:
            Logger.warning(f"User story {user_story['id']} not found in the database. Skipping rework process.")
            return
        ...
        Logger.info(f"User story {user_story['id']} - Current status: {current_status_name}, New status: {new_status_name}")
        
        if user_story['id'] == 7867302:
            print(f"User story {user_story['id']} - Current status: {current_status_name}, New status: {new_status_name}")
        ...
        if status_weights.get(new_status_name) == status_weights.get(current_status_name):
            Logger.info(f"User story {user_story['id']} - Status unchanged. No action taken.")
            return

        if status_weights.get(new_status_name) > status_weights.get(current_status_name):
            # Avanço de status
            Logger.info(f"User story {user_story['id']} - Status advanced from {current_status_name} to {new_status_name}. Updating database.")
            try:
                update_status = (
                    "UPDATE public.dim_user_story SET id_status = %s WHERE id_taiga = CAST(%s AS BIGINT)"
                )
                cursor.execute(update_status, (internal_new_status_id, user_story["id"]))
                conn.commit()
                Logger.info(f"User story {user_story['id']} - Status updated successfully.")
            except Exception as e:
                Logger.error(f"Error updating status for user story {user_story['id']}: {e}")

        if status_weights.get(new_status_name) < status_weights.get(current_status_name):
            # Retrabalho detectado
            Logger.warning(f"User story {user_story['id']} - Rework detected! Status regressed from {current_status_name} to {new_status_name}. Logging rework and updating database.")
            try:
                insert_rework = (
                    "INSERT INTO public.dim_status_historico (id_user_story, data_retrabalho, id_status) VALUES (CAST(%s AS BIGINT), NOW(), %s)"
                )
                cursor.execute(insert_rework, (internal_user_story_id, internal_new_status_id))
                conn.commit()
                Logger.info(f"User story {user_story['id']} - Rework logged in dim_status_historico.")
            except Exception as e:
                Logger.error(f"Error logging rework for user story {user_story['id']}: {e}")
            try:
                update_status = (
                    "UPDATE public.dim_user_story SET id_status = %s WHERE id_taiga = CAST(%s AS BIGINT)"
                )
                cursor.execute(update_status, (internal_new_status_id, user_story["id"]))
                conn.commit()
                Logger.info(f"User story {user_story['id']} - Status updated after rework.")
            except Exception as e:
                Logger.error(f"Error updating status after rework for user story {user_story['id']}: {e}")
    except Exception as e:
        Logger.error(f"Error processing rework logic for user story {user_story['id']}: {e}")