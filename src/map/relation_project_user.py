...
def upsert_relation_project_user():
    ...
    try:
        ...
        for project in projects:
            ...
            try:
                Logger.debug(
                    f"Executing query to find project ID: {select_internal_project_id} with parameter: {project['name']}"
                )
                ...
                Logger.debug(
                    f"Executing query to find user ID: {select_internal_user_id} with parameter: {project['owner']['id']}"
                )
                ...
                if taiga_user_id is None:
                    Logger.warning(
                        f"Assigned user is None for project {project['id']}... skipping"
                    )
                    continue
                ...
                if not internal_user_id:
                    Logger.warning(
                        f"User '{user_complete['email']}' not found in the database. Skipping."
                    )
                    continue

                Logger.debug(
                    f"Inserting relationship with query: {insert_relation} and parameters: {internal_user_id[0]}, {internal_project_id[0]}"
                )
                cursor.execute(
                    insert_relation, (internal_user_id[0], internal_project_id[0])
                )
                conn.commit()
                Logger.info(
        ...
        Logger.error(
            f"Error establishing database connection or processing projects: {str(e)}"
        )
    finally:
        if conn:
