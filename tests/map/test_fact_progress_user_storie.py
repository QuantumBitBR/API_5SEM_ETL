...
class TestFactProgressUserStorie(unittest.TestCase):
    ...
    def test_process_fact_progress_user_storie(self, mock_upsert, mock_zero, mock_extract, mock_logger):
        # Arrange
        ...
        # Assert
        mock_logger.info.assert_any_call("Starting upsert_fact_progress_user_storie process")
    ...
    @patch('src.map.fact_progress_user_storie.Logger')
    def test_extract_data_2_fact_progress_user_storie(self, mock_logger, mock_database, mock_get_user_storys_by_project, mock_get_all_projects):
        # Arrange
        ...
        mock_logger.info.assert_any_call("Starting extract_data_2_fact_progress_user_storie process")
        mock_logger.info.assert_any_call("Extracted 1 user stories from project Project 1 (ID: 1)")
        mock_logger.info.assert_any_call("Extracted 1 user stories from project Project 2 (ID: 2)")
        mock_logger.info.assert_any_call("Fetched internal project ID: 1 for project Project 1")
        mock_logger.info.assert_any_call("Fetched internal tag ID: 1 for tag tag1")
        mock_logger.info.assert_any_call("Fetched internal status ID: 1 for status status1")
        mock_logger.info.assert_any_call("Fetched internal project ID: 2 for project Project 2")
        mock_logger.info.assert_any_call("Fetched internal tag ID: 2 for tag tag2")
        mock_logger.info.assert_any_call("Fetched internal status ID: 2 for status status2")
        ...
        mock_logger.info.assert_any_call("Database connection closed for project processing")
