...
class TestProcessFactTagUserStorie(unittest.TestCase):
   ...
   def test_process_fact_tag_user_storie(self, mock_upsert, mock_zero, mock_extract, mock_logger):
      # Arrange
   ...
   def test_zero_all_progress(self, mock_logger, mock_database):
      ...
      # Assert
      mock_cursor.execute.assert_called_once_with("UPDATE public.fato_tag_user_story SET quantidade_user_story = 0")
   ...
   def test_zero_all_progress_exception(self, mock_logger, mock_database):
      ...
      # Assert
      mock_logger.error.assert_called_once_with("An error occurred while updating progress quantities: Test exception")
      mock_cursor.close.assert_called_once()
      mock_database.return_value.release_connection.assert_called_once_with(mock_conn)
      mock_logger.info.assert_any_call("Database connection closed")
